...
var findAllConcatenatedWordsInADict = function (words) {
    ...
    const dfs = (word, start) => {
        ...
        for (let i = start; i < word.length; i++) {
            ...
            root = wordTree.children[index];
            if (root == null) {
                return false;
            }
            if (root.isLeaf && dfs(word, i + 1)) {
                return true;
            }
        }
        return false;
    }
    return words.reduce((pre, word) => {
        if (word.length !== 0 && dfs(word)) {
            pre.push(word)
        } else {
            wordTree.add(word);
        }
...
class WordTree {
    ...
    add(word) {
        ...
        for (let i = 0; i < word.length; i++) {
            let index = word[i].charCodeAt(0) - 97;
            if (!root.children[index]) root.children[index] = new WordTree();
...
// console.log(tree.search('hippopotamuses'))
