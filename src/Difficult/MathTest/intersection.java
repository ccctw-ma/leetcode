...
public class intersection {
    ...
    public double[] intersection2(int[] start1, int[] end1, int[] start2, int[] end2) {
        int fsx=start1[0],fsy=start1[1];
        int fex=end1[0],fey=end1[1];
        int ssx=start2[0],ssy=start2[0];
        int sex=end2[0],sey=end2[1];
        Map<Integer,Integer> map=new HashMap<>();
        map.put(fsx,fsy);
        map.put(fex,fey);
        map.put(ssx,ssy);
        map.put(sex,sey);
        if(fsx==fex&&ssx==sex&&fsx==ssx){
            double y=Math.min(Math.min(fsy,fey),Math.min(ssx,sey));
            double[] doubles;
            doubles = new double[]{(double) fsx, y};
            return doubles;
        }
        if((fsy==fey)&&(ssy==sey)&&(fsy==sey)){
            double x=Math.min(Math.min(fsx,fex),Math.min(ssx,sex));
            double[] doubles;
            doubles = new double[]{x, (double) fsy};
            return doubles;
        }
        double a,b,c,d;
        if((fey-fsy)*(sex-ssx)==(sey-ssy)*(fex-fsx)){
            if((fex-ssx)*(sex-fsx)>=0&&(fey-fsy)*(sex-ssx)==(fex-fsy)*(fex)){
                double x=Math.min(Math.min(fsx,fex),Math.min(ssx,sex));
                return new double[]{x,(double)map.get(x)};
            }
        }else {
